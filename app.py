...
if not st.session_state.user_email:
    st.markdown("<h1 style='text-align:center;font-size:3rem;'>📊 BlackScholes.ai</h1>", unsafe_allow_html=True)
    st.markdown("<p style='text-align:center;font-size:1.6rem;color:#00ff9d;'>The equation that created the multi-trillion dollar derivatives market — now in your browser.</p>", unsafe_allow_html=True)
    st.latex(r"\frac{\partial V}{\partial t} + rS \frac{\partial V}{\partial S} + \frac{1}{2} \sigma^2 S^2 \frac{\partial^2 V}{\partial S^2} - rV = 0")
    
    st.info("**Free:** 5 calculations/day  **Pro:** $9.99/month unlimited")
    ...
    with tab1:
        st.subheader("Options Calculator + Smart Buy/Sell Signal")
        ...
        if st.button("🚀 Calculate & Analyze", type="primary", use_container_width=True):
            if not st.session_state.is_pro and st.session_state.calculations_used >= 5:
                ...
                if market_price < fair_price * 0.95:
                    st.success("🟢 **STRONG BUY** — Option is significantly undervalued")
    ...
    with tab2:
        ...
        st.info("More advanced strategy builder with full payoff charts coming in the next update.")

    with tab3:
        st.subheader("Portfolio Tracker")
        st.info("Portfolio tracker with total Greeks and risk metrics coming soon.")
